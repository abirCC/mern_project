...
export default function Signup() {
  const [credentials,setcredentials] = useState({name:"",email:"",password:"",geoloacation:""})
  const handleSubmit=async(e)=> {
      ...
      const response = await fetch("http://localhost:5000/api/createuser",{
        ...
        body:JSON.stringify({name:credentials.name,email:credentials.email,location:credentials.geoloacation,password:credentials.password})
       });
      ...
      <div className="mb-3">
        ...
        <div className="mb-3">
          ...
          <input
            type="text"
            className="form-control"
            name='geoloacation' value={credentials.geoloacation} onChange={valChange}
          />
        </div>
